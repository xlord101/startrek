...
interface HarvestingClientProps {
  supervisors: User[];
...
export default function HarvestingClient({
  ...
  // Hydrate store with server-rendered tasks BEFORE first paint
  useLayoutEffect(() => {
    if (initialTasks?.length) {
      store.setHarvestTasks(initialTasks as any);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const fetchHarvestData = useCallback(() => {
    fetch("/api/harvest")
      .then((r) => {
